...
\]
in distributions. On each bounded interval $I$,
\[
...
We continue our program by proving Corollary~\ref{cor:cubic-NLS}.  We recall, for the reader's convenience, the non-linear cubic Sch\"odinger equation 
\[
 i\partial_t w+\partial_x^2w=\sigma|w|^2w.
 \tag{2.1}\label{eq:cubic-NLS}
\]
pertaining to that result. 

\begin{proof}[Proof of Corollary~\ref{cor:cubic-NLS}]
Our main strategy will be to verify that the equation, as well as the `potential' $V = |u|^2 = |v|^2$, satisfy the hypotheses of Theorem \ref{thm:time-potential}. Fix then a bounded interval $I$. On a time interval $K$, the homogeneous and
