...
In particular, the potentials in~\cite{Jaming2025} are all assumed to be time-\emph{independent}, and hence Theorem \ref{thm:time-potential} provides a positive answer to Question I.2 in~\cite{Jaming2025} for a wide class of potentials, namely any potential in $L^2(\R)+L^\infty(\R)$, which includes all $L^p(\R)$,
$2\leq p\leq\infty$.

The proof of Theorem \ref{thm:time-potential} starts with the endpoint Strichartz and local smoothing estimates. On each compact time interval, these give
\[
...
\]
This is enough to justify the continuity equation at the $L^2$ endpoint. Since $|u|=|v|$, the two solutions have the same current. A regularization argument which does not divide by either solution at its zeros then gives
\[
...
The next step is to introduce the exterior product
\[
 F(t,x,y)=u(t,x)v(t,y)-v(t,x)u(t,y).
\]
It solves a two-dimensional Schr\"odinger equation with potential $V(t,x)+V(t,y)$. In the coordinates $r=(x+y)/\sqrt2$ and $s=(x-y)/\sqrt2$, the function $F$ is odd in $s$. Its Dirichlet trace at $s=0$ vanishes by oddness, and its normal trace vanishes by the Wronskian identity. The zero extension of $F$ from $s>0$ therefore solves the same equation without a boundary source.

Finally, a stripwise version of the Ionescu--Kenig Carleman estimate propagates this zero half-space through the whole $(r,s)$-plane. The $L^1_tL^\infty_x$ part of the potential is made small by shortening the time interval, while the restriction of the $L^2_{t,x}$ part to a strip of width $h$ has norm of order $h^{1/2}$. Taking $h$ small permits iteration of the estimate and gives $F=0$. Thus $u$ and $v$ are linearly dependent at one time; equality of their moduli makes the proportionality constant unimodular, and uniqueness propagates the same phase throughout $I_0$.
...
\]
by H\"older's inequality and $w\in C(I;L^2)$.  The Duhamel integral is
...
\end{align*}
Apply these estimates to the Duhamel formula.  For either solution $w$, this
gives
\begin{align*}
...
the limit proves $q\,\partial_xp-p\,\partial_xq=0$.  Apply this with
$p=u(t)$ and $q=v(t)$ on the common full-measure set supplied by
\eqref{eq:linear-endpoint-regularity} and the first identity in
\eqref{eq:current-wronskian}.
\end{proof}
