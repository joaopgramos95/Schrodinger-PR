...
By exploiting and exhausting several paths towards the general $L^2$ case with GPT 5.4 and 5.5, as well as Claude Opus 4.6, 4.7, 4.8 and Fable 5.0, we had the idea of working at the level of currents. By doing so, we were naturally led to the idea of lifting our equation one dimension higher. The details on how to finish from there, such as applying specific unique continuation lemmas, were then provided by several different LLMs. Finally, Claude Fable 5.0 was also used in order to obtain a lean verification of Corollary \ref{cor:cubic-NLS}; see (add). 

Most of the other attempts and results through different methods have been written down and are kept in the following repository (add). Apart from that note -- which is mostly LLM-written --, the results and proofs here have all been written and verified by humans. We take full responsibility for the contents of both manuscripts. 
