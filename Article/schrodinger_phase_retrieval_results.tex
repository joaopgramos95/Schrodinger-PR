...
$L^1_tL^\infty_x+L^2_{t,x}$; no spatial decay is required.  The result yields
phase retrieval for the cubic nonlinear Schr\"odinger equation in the global
$L^2$ class.  The conclusion is false for the free equation in every spatial
dimension at least two, even for Schwartz initial data.
\end{abstract}
...
Let $I_0\subset\mathbb R$ be a nonempty open interval.  We consider two
solutions of
\[
 if_t+f_{xx}=Vf
\]
with the same real potential.  The principal result requires neither spatial
decay of $V$ nor finite energy of the solutions.

\begin{mainresult}[Common time-dependent potentials]
...
\]
Let $u,v\in C(I_0;L^2(\mathbb R))$ be mild solutions of
\[
 if_t+f_{xx}=Vf.
\]
If
...
If the potential and both solutions extend globally, uniqueness propagates
the conclusion to every time.  The theorem also applies to nonlinear
equations whenever equality of the moduli makes their nonlinear potentials
identical.  This gives the following consequence.
...
The one-dimensional restriction cannot be removed.
...
The proof of Theorem~\ref{thm:time-potential} uses the Strichartz and
local-smoothing estimates and a stripwise form of the Ionescu--Kenig Carleman
estimate.  These three external inputs are stated in the forms needed below.
Every deduction from them is proved.

All nonsmooth identities are distributional unless another meaning is
specified.  If $f\in H^{1/2}_{\mathrm{loc}}(\mathbb R)$, products such as
$\overline f f_x$ are interpreted by localized
$H^{-1/2}$--$H^{1/2}$ duality.  We take Hilbert-space inner products to be
