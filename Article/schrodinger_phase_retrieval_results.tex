...
function $V:I_0\times\mathbb R\to\mathbb R$ admits, on every compact sub-interval
...
Put $\mathcal H=i\partial_t+\Delta_z$.  Put
\begin{align*}
...
cannot increase any of these three norms.  For a sum space, this follows
by multiplying each term of a decomposition and then taking the infimum.
For the intersection space, it follows by bounding each of its two norms.
...
\]
No decomposition attaining the infimum is needed.
...
\[
 ...
 \qquad 0<\tau<1.
\]
This follows because both the time derivative and the spatial Laplacian
acquire the factor $L$.  In two spatial dimensions the change of variables
satisfies $d\tau\,d\xi=L^{-2}\,dt\,dz$.  For any decomposition $A=A_\infty+A_2$ of a potential or restricted
potential, write
...
\begin{align*}
 ...
 &=\|A_2\|_{L^2_{t,z}}.
\end{align*}
We do not assume $W\in Y$ globally.
Transforming decompositions in both directions shows that the $Y$ norm is
also preserved.  The forcing is transformed with the factor $L^{3/2}$,
which preserves both summand norms in $X$.  Thus all the hypotheses are
preserved, with $b$ and $h$ replaced by $b/L^{1/2}$ and $h/L^{1/2}$.
It suffices to work on $J=[0,1]$.

We may also decrease $h$ so that $0<h\le1$.  To justify this, let
$0<\ell\le h$ and $a\le b-\ell$.  The strip $(a,a+\ell)$ is contained in
the strip $(\alpha,\alpha+h)$, where $\alpha=\min\{a,b-h\}$.
Indeed, if $a\le b-h$ we take $\alpha=a$; otherwise
$(a,a+\ell)\subset(b-h,b)$.  Since $\alpha\le b-h$, the original
hypothesis and the restriction property from Step 1 give the same
smallness bound for width $\ell$.  We take $\ell=\min\{h,1\}$ and denote
this new width by $h$.

\medskip
\noindent\emph{Step 3: the Carleman inequality.}
Put $\mathcal H_y=i\partial_t+\Delta_y$.  For $\beta\ge2$, define
\[
 \phi_\beta(r)=
 \begin{cases}
  \beta r,&r\ge-1,\\
...
\]
For $r=-s\in[-1,0]$, the second bound follows from
$(1+s)e^{-\beta s}\le e^{-(\beta-1)s}\le1$.
For $s>1$, it follows from
$(1+s)e^{-s-\beta+1}\le e^{1-\beta}\le1$.
Here we used $1+s\le e^s$ for $s\ge0$.

The cited lemma gives absolute constants $C_0\ge1$ and $c_0>0$ such that
\begin{align*}
 &\|\1_{\{0<y_1<1\}}e^{\phi_\beta(y_1)}U\|_{X'}
 +\beta^{c_0}
  \|\1_{\{0<y_1<1\}}e^{\phi_\beta(y_1)}U\|_{L^1_tL^2_y}\\
 &\qquad\le C_0\Bigl(
 \|e^{\phi_\beta(y_1)}\mathcal H_yU\|_X
...
Moreover, $U_\delta\in C([0,1];L^2)$: spatial translations and
multiplications by $e^{iay_1}$ depend strongly continuously on their real
parameters in $L^2$.  For translations this follows first for smooth
compactly supported functions and then by density; for the phase factors
it follows from dominated convergence.  Combining these facts with the
continuity of $Z$, $w_\delta$, and $w_\delta'$ proves the assertion.
...
\[
 \mathcal H_y\widetilde Z_\delta
...
\[
 \mathcal H_yU_\delta=(W_\delta+M_\delta)U_\delta,
 \qquad
 M_\delta(t,y)=\tfrac12w_\delta''(t)y_1
               -\tfrac14(w_\delta'(t))^2.
 \tag{1.4c}\label{eq:IK-moving-equation}
\]
The computation also holds for our nonsmooth $Z$.  Indeed, the translation
is a smooth change of spacetime variables with Jacobian one, and the phase
factor is smooth.  Testing against a compactly supported smooth function,
changing variables, and integrating by parts gives the same derivative
identities in distributions.  This uses no classical derivatives of $Z$.
...
\]
The norm equality follows by translating and multiplying each summand in
an $X$ decomposition, and applying the inverse transformation for the
reverse inequality.  Since
\[
...
$(1+|y_1|)^{-1}\mathcal H_yU_\delta\in X$.
...
almost everywhere.  Time-dependent spatial translation preserves both
summand norms in $Y$, and restriction cannot increase them.  Therefore
\[
...
\]
This explains the absorption: the same nonnegative norms occur on both
sides, and their coefficients on the right are strictly smaller.
Only the strip restriction of $W$ was required to be small.  The part
outside that strip was estimated through the assumed finite norm
$\|WZ\|_X$, not through a global $Y$ norm of $W$.
...
\]
Undoing the change of scale from Step 2 proves the conclusion on the
original interval $J$.
\end{proof}
...
theory~\cite{DeiftTrubowitz,EgorovaKopylovaMarchenkoTeschl} gives finitely many simple negative eigenvalues
...
has no positive-energy zeros~\cite{DeiftTrubowitz}, so the relation on the free half-line recovers
