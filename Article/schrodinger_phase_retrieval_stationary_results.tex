...
\begin{abstract}
We prove three phase-retrieval results for stationary one-dimensional
Schr\"odinger evolutions.  Masuda's unique-continuation theorem yields phase
retrieval for finite-energy solutions and a broad class of semibounded
potentials which may grow at infinity.  For real potentials in the Faddeev
class $L^1_1(\mathbb R)$, measurements on all of spacetime determine arbitrary
$L^2$ initial data.  If the potential is smooth and compactly supported to the
left of a point, measurements on the exterior half-line to its right suffice.
\end{abstract}
...
Let $V:\R\to\R$, let $H=-\partial_x^2+V$ be a self-adjoint
Schr\"odinger operator, and write
\[
 u(t)=e^{-itH}u_0,\qquad v(t)=e^{-itH}v_0.
\]
We ask whether equality of $|u|$ and $|v|$ determines the initial data up to
one constant in $\T=\{z\in\C:|z|=1\}$.  This note gives three answers for
time-independent potentials.  We state first the argument based on Masuda's
unique-continuation theorem.

\begin{mainresult}[Masuda class]
\label{thm:phase-retrieval}
Let $V\in L^\infty_{\mathrm{loc}}(\R;\R)$.  Assume that
\[
 H_0=-\partial_x^2+V,\qquad D(H_0)=C_c^\infty(\R),
\]
is essentially self-adjoint and its self-adjoint closure is bounded from
below.  Let $u$ and $v$ be weak solutions of
\[
 if_t+f_{xx}=Vf,
 \qquad u,v\in C(\R;L^2(\R)).
\]
Suppose that, on a nonempty open interval $I\subset\R$,
\[
 u,v\in C(I;H^1(\R)),\qquad
 |u(t,x)|=|v(t,x)|
 \quad\text{for almost every }(t,x)\in I\times\R.
\]
Then $v(t)=\zeta u(t)$ in $L^2(\R)$ for every $t\in\R$ and some
$\zeta\in\T$.
\end{mainresult}

The second theorem permits arbitrary $L^2$ data.  Set
\[
...
\label{thm:main}
Let $V\in L^1_1(\R)$ be real, let $H=-\partial_x^2+V$ be its
self-adjoint quadratic-form realization, and let $u_0,v_0\in L^2(\R)$.  If
\[
 |e^{-itH}u_0(x)|=|e^{-itH}v_0(x)|
 \quad\text{for almost every }(t,x)\in\R^2,
\]
then $v_0=\zeta u_0$ for some $\zeta\in\T$.
\end{mainresult}

For a smooth compactly supported potential, observation on the free side of
the potential suffices.

\begin{mainresult}[Observation on one exterior half-line]
\label{thm:half-line}
Let $V\in C_c^\infty(\R;\R)$, let $H=-\partial_x^2+V$, and choose
$R\in\R$ so that $\operatorname{supp}V\subset(-\infty,R]$.  If
$u_0,v_0\in L^2(\R)$ and
\[
 |e^{-itH}u_0(x)|=|e^{-itH}v_0(x)|
 \quad\text{for almost every }(t,x)\in\R\times(R,\infty),
\]
then $v_0=\zeta u_0$ for some $\zeta\in\T$.
\end{mainresult}

The first theorem allows locally bounded semibounded potentials which may
grow at infinity, but it assumes finite energy on the observation interval.
The second assumes spatial decay and observations on all of spacetime, but it
allows arbitrary $L^2$ data, negative eigenvalues, and a zero-energy
resonance.  The third strengthens the observation geometry under the more
restrictive hypothesis $V\in C_c^\infty$.

The proofs use three different facts.  Equality of densities determines the
one-dimensional probability current, after which Masuda's theorem propagates
a local phase globally.  In the Faddeev class, the temporal Fourier transform
separates pairs of spectral energies by their difference; a four-dimensional
first-order product system then separates the remaining energy sum.  On an
exterior half-line, the spectral expansion is free, and a Cauchy-transform
argument recovers its rank-one spectral kernel.

All nonsmooth identities below are distributional unless another meaning is
specified.  We use
\[
 \widehat f(\xi)=(2\pi)^{-1/2}\int_\R e^{-ix\xi}f(x)\,dx
\]
...
\section{Phase retrieval in Masuda's class}\label{sec:masuda}

Throughout this section $V$ satisfies the hypotheses of
Theorem~\ref{thm:phase-retrieval}.  Masuda's theorem~\cite{Masuda} is stated
with a null exceptional set $Q$ whose complement is connected and open and on
which $V$ is locally bounded.  In one dimension this complement must equal
$\R$: a connected open subset of $\R$ is an interval, and a proper interval
cannot have full measure.  Thus the exceptional-set hypothesis reduces to
$V\in L^\infty_{\mathrm{loc}}(\R)$.  Masuda also allows a real $C^2$
magnetic potential, but we only use the nonmagnetic case.
...
\subsection{Density and current}
...
\begin{equation}
 ...
 \label{eq:equal-current}
\end{equation}
Consequently,
\begin{equation}
 \overline u\,u_x=\overline v\,v_x
 \quad\text{almost everywhere on }I\times\R.
 \label{eq:equal-logarithmic-derivative}
\end{equation}
\end{lemma}
...
\subsection{Local constancy of the relative phase}
...
$\Omega_0$ of spacetime.  Proposition~\ref{prop:masuda} gives
$w\equiv0$ on $\R^2$.  Thus
$v(t)=\zeta u(t)$ in $L^2(\R)$ for every $t\in\R$.
\end{proof}


\section{Stationary potentials in the Faddeev class}\label{sec:faddeev}
...
\begin{equation}
 ...
 \qquad
 |\partial_x e_\alpha(x,k)|\lesssim 1+k.
 \label{eq:scattering-local-bounds}
\end{equation}
...
\begin{equation}
 ...
 \label{eq:uniform-scattering-expansion}
\end{equation}
and
\[
...
\begin{equation}
 \phi_j(x)=a_j^+e^{-\kappa_j x}(1+o(1))
 \quad (x\to+\infty),
 \label{eq:bound-tail-plus}
\end{equation}
...
Let $f_\pm(x,k)\sim e^{\pm ikx}$ as $x\to\pm\infty$ be the Jost
solutions.  We choose the physical incidence
...
\subsection{A fixed-gap estimate}
...
\subsection{The product system}

The following elementary system is the central structural observation.

\begin{lemma}[Product system]\label{lem:product-system}
Let $V\in L^1_{\mathrm{loc}}(\R)$, let $E,F\in\C$, and suppose that
$y,z\in W^{2,1}_{\mathrm{loc}}(\R)$ satisfy
\[
 -y''+Vy=Ey,
 \qquad
 -z''+Vz=Fz
...
\begin{equation}
 ...
 \quad
 D=y'z'.
 \label{eq:product-state-definition}
\end{equation}
...
\begin{equation}
 ...
 \end{aligned}}
 \label{eq:product-system}
\end{equation}
\end{lemma}

\begin{proof}
The functions $y,z$ and their first derivatives are locally absolutely
continuous, so the ordinary product rule is valid almost everywhere.  Since
\[
 y''=(V-E)y,
 \qquad z''=(V-F)z,
\]
...
\begin{equation}
 ...
 \label{eq:tensor-identification}
\end{equation}
...
\begin{equation}
 ...
 \label{eq:negative-growing-solution}
\end{equation}
...
\[
 ...
 \quad (|x|\ge R).
\]
Hence $y,y'\in L^2(\R)$, so $y\in H^1(\R)$.  Since
$V\in L^1(\R)$ and $H^1(\R)\hookrightarrow L^\infty(\R)$,
\[
 \int_\R |V(x)|\,|y(x)|^2\,dx
 \le \|V\|_1\|y\|_\infty^2<\infty.
\]
Thus $y$ belongs to the form domain of the quadratic-form realization of
$H$.  The distributional equation gives, first for
$\varphi\in C_c^\infty(\R)$,
\[
 \int_\R y'\overline{\varphi'}+Vy\overline\varphi
 =G\int_\R y\overline\varphi.
\]
Both sides are continuous in the form norm, so the identity extends to the
whole form domain.  The representation theorem for closed semibounded forms
then gives $y\in D(H)$ and $Hy=Gy$.  Hence $G$ is an $L^2$ eigenvalue.
\end{proof}

\subsection{Fixed-gap injectivity}
...
\begin{equation}
 ...
 \end{aligned}}
 \label{eq:lambda-product-system}
\end{equation}
...
\begin{equation}
 ...
 \label{eq:negative-lambda-choice}
\end{equation}
...
\begin{equation}
 ...
 \label{eq:negative-resolvent-zero}
\end{equation}
...
\subsection{The temporal Fourier transform of the density}
...
\smallskip
\noindent
\emph{Step 1: approximation of the spectral density.}
Define
\begin{equation}
 Q_{\alpha\beta}(k,\ell)
 =g_\alpha(k)\overline{g_\beta(\ell)}
  -h_\alpha(k)\overline{h_\beta(\ell)}.
 \label{eq:ac-kernel-difference}
\end{equation}
...
\begin{equation}
 ...
 \longrightarrow0,
 \label{eq:density-approximation}
\end{equation}
...
\begin{equation}
 ...
 \right),
 \label{eq:double-form-approximation}
\end{equation}
...
\smallskip
\noindent
\emph{Step 2: disintegration by the energy difference.}
In the continuous--continuous term, apply the change of variables
\[
 \delta=k^2-\ell^2,
 \qquad s=k^2+\ell^2.
\]
...
\begin{equation}
 ...
 \label{eq:fixed-gap-common-null-set}
\end{equation}
...
\smallskip
\noindent
\emph{Step 3: the bound-state block.}
The part just analyzed has an $L^1_{\mathrm{loc}}$ temporal-frequency density,
so it has no
...
\begin{equation}
 ...
 \label{eq:bound-kernel-recovered}
\end{equation}

\subsection{Recovery of the initial state}
...
\begin{align*}
 U(t,y)&=\sum_{j=1}^N a_j e^{-\kappa_j y}e^{i\kappa_j^2t}
          +\mathcal E_t F(y),\\
 W(t,y)&=\sum_{j=1}^N b_j e^{-\kappa_j y}e^{i\kappa_j^2t}
          +\mathcal E_t G(y)
\end{align*}
...
\begin{proof}
\emph{Step 1: time continuity.}
Plancherel's theorem gives
$\|\mathcal E_t F\|_{L^2(\R)}=\sqrt{2\pi}\|F\|_2$, independently of $t$.
Moreover, dominated convergence in Fourier space shows that
$t\mapsto\mathcal E_t F$ is continuous into $L^2(\R)$.  The finite bound-state
...
\emph{Step 2: a Cauchy-transform uniqueness statement.}
Let $J\subset\R$ be a bounded open interval, let $m\in L^1(J)$, and suppose
\[
...
\begin{align*}
 ...
 &\qquad=-2\pi i\int_J m(\eta)(P_\varepsilon*\phi)(\eta)\,d\eta,
 \qquad
...
$-2\pi i\int_J m(\eta)\phi(\eta)\,d\eta$.  Thus $m$ vanishes as a
distribution on every $K\Subset J$, and hence almost everywhere on $J$.

\emph{Step 3: the time--space transform.}
...
\begin{equation}
 ...
 \label{eq:half-line-frequency-selector}
\end{equation}
...
\[
 \int_0^\infty e^{-ay}|\mathcal E_t F(y)|^2\,dy
 \le\|\mathcal E_t F\|_2^2=2\pi\|F\|_2^2
\]
...
$\mathbf 1_{\{|\ell^2-k^2|\le M\}}$ has uniformly bounded integrals in each
...
\emph{Step 4: recovery of the continuous spectral kernel.}
We separate the free--free term from the others by analytic continuation.  The numerator of $\Psi^+_{j,\chi}$ has compact $k$-support.
...
\emph{Step 5: recovery of the bound-state coefficients.}
First suppose $F\ne0$.  Replace $W$
...
\begin{equation}
 \sum_{j=1}^N C_{j,\chi}(z)=0,
 \qquad
 C_{j,\chi}(z)=\int_\R
 \frac{(a_j-b_j)\overline{F(k)}\chi(k^2+\kappa_j^2)}
      {z+\kappa_j+ik}\,dk.
 \label{eq:bound-induction}
\end{equation}
...
\[
 C_{1,\chi}(z)=-\sum_{m=2}^N C_{m,\chi}(z).
\]
...
\[
 \sum_{j,m=1}^N d_{jm}e^{-(\kappa_j+\kappa_m)y}
...
therefore
\[
 \phi_j(x)=\alpha_j e^{-\kappa_jx},\qquad \alpha_j\ne0.
\]
...
$\phi_j\equiv0$.  The continuum functions may be normalized so that
\[
 e_1(x,k)=T(k)e^{ikx},
 \qquad
...
\begin{align*}
 e^{-itH}u_0(x)
 ={}&\sum_{j=1}^N c_j\alpha_j e^{-\kappa_jx}e^{i\kappa_j^2t}\\
 &+\int_0^\infty e^{-ik^2t}
